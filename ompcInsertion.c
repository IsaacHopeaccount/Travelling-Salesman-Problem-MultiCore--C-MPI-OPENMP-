...
// Function to find the tour using the cheapest insertion heuristic for a given starting vertex.
void cheapestInsertionStartingFrom(double **distanceMatrix, int numOfCoords, int *tour, int startVertex) {
...
int vk=0,vn=0;
//#pragma omp parallel for private(vk, vn)
        for (int vk = 0; vk < numOfCoords; vk++) {
            if (!visited[vk]) {
                for (int vn = 0; vn < currentPosition; vn++) {
                    ...
                    // Update the best vertex and position if the current insertion cost is smaller.
//#pragma omp critical
        ...
        // Update the tour with the best vertex at the best position.
...
int cheapestInsertion(char *argv[], double **distanceMatrix, int numOfCoords, int startVertexFrom, int startVertexTo)
{
